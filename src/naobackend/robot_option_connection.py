...
class RobotOptionFloat(RobotOption):
    ...
    def __init__(self, option_sender: Callable[[str, bytes], None], name: str, min: float, max: float, step: float,
                 value: float):
        self.name = name
        self.min = min
        self.max = max
...
class RobotOptionSet:
    ...
    def __init__(self, fp: io.BufferedIOBase, option_sender: Callable[[str, str, bytes], None]):
        ...
        def my_option_sender(option_name: str, option_contents: bytes):
            option_sender(self.name, option_name, option_contents)
