...
[[nodiscard]] std::thread launch_named_thread(const std::string& name, bool low_prio, Callable&& callable, Args&&... args) {
    return std::thread([&name, callable, args...]() {
        ...
        pthread_setname_np(pthread_self(), (new std::string(name))->c_str());
...
template <typename T>
[[nodiscard]] T normalizeRotation(T val) {
    while (val > M_PI)
        val -= M_PI * 2.f;
    while (val < -M_PI)
        val += M_PI * 2.f;
...
typename It::value_type angleAvg(const It& begin, const It& end) {
    typename It::value_type cos_sum = 0;
    typename It::value_type sin_sum = 0;
    for (It it = begin; it != end; it++) {
        cos_sum += cos(*it);
        sin_sum += sin(*it);
    }
    size_t size = std::distance(begin, end);
    return atan2(sin_sum / size, cos_sum / size);
}
...
inline float deadband_filter(float min, float value, float max) {
    if (value < 0) {
        value -= min;
        if (value > 0) {
            value = 0;
